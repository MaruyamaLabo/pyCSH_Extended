...
def get_coordinates_brick( atom_index, atom_index_cshff, bond_index, bond_index_cshff, pieces, v_cell, supercell, supercell_inv, brick, water_in_brick ):
	...
	if any(SiO2 in brick.comb for SiO2 in ['SU', 'SUo']):
		Up_bridging = True
	elif any(SiO2 in brick.comb for SiO2 in ['SD', 'SDo']):
		Down_bridging = True
	...
	for piece in brick.comb:
		...
		for iat in range( pieces[piece].N_atom ):
			...
			if Up_bridging:
				entry = [atom_index, specie, charges[specie], r[0], r[1], r[2], piece, atom_index_cshff, iat, "Up-bridging"] #Umeki_241031
			elif Down_bridging:
				entry = [atom_index, specie, charges[specie], r[0], r[1], r[2], piece, atom_index_cshff, iat, "Down-bridging"] #Umeki_241031
			else:
				entry = [atom_index, specie, charges[specie], r[0], r[1], r[2], piece, atom_index_cshff, iat, "non-bridging"] #Umeki_241031
			...
			if specie == 6:
				...
				entry = [atom_index, 8, charges[8], r_H[0], r_H[1], r_H[2], piece, atom_index_cshff, iat, "non-bridging"]
				at_entries.append(entry)
				...
				entry = [ bond_index_cshff, 1, atom_index_cshff-1, atom_index_cshff]
				bd_entries_cshff.append(entry)
			...
			if specie == 3:
				entry = [ atom_index, 4, charges[4], r[0], r[1], r[2]+0.05, piece, atom_index_cshff, iat, "non-bridging"]
				at_entries.append(entry)
				piece_entries.append(entry)

				entry = [ bond_index, 3, atom_index-1, atom_index]
				bd_entries.append(entry)
				# cshffはO(S)がないのでbd_entries_cshffは更新しない

				bond_index += 1
				# cshffはO(S)がないのでbond_index_cshffは更新しない
				atom_index +=1
				# cshffはO(S)がないのでatom_index_cshffは更新しない
	...
	for w in water_in_brick:
		...
		entry = [ atom_index, specie, charges[specie], r[0], r[1], r[2], piece, atom_index_cshff]
		...
		r_H = r + pieces[w].r_H1
		#r_H = apply_PBC(r_H, supercell, supercell_inv)
		entry = [ atom_index, 7, charges[7],  r_H[0], r_H[1], r_H[2], piece, atom_index_cshff]
		...
		bd_entries.append(entry)
		entry = [ bond_index_cshff, 2, atom_index_cshff-1, atom_index_cshff]
		bd_entries_cshff.append(entry)
		...
		bd_entries.append(entry)
		entry = [ bond_index_cshff, 2, atom_index_cshff-2, atom_index_cshff]
		bd_entries_cshff.append(entry)
	...
	return at_entries, atom_index, atom_index_cshff, bd_entries, bd_entries_cshff, bond_index, bond_index_cshff, brick_dict, water_dict
	...
	for i in range(shape[0]):
		for j in range(shape[1]):
			for k in range(shape[2]):
				...
				at_entry_b, atom_index, atom_index_cshff, bd_entry_b, bd_entries_cshff_b, bond_index, bond_index_cshff, brick_dict, brick_water_dict =  get_coordinates_brick( 
											atom_index, atom_index_cshff, bond_index, bond_index_cshff, pieces, v_cell, supercell, supercell_inv, 
											crystal_rs[i,j,k], water_in_crystal_rs[i,j,k] )
				
				atom_entries = atom_entries + at_entry_b
				bond_entries = bond_entries + bd_entry_b
				bond_entries_cshff = bond_entries_cshff + bd_entries_cshff_b

				crystal_dict[(i,j,k)] = brick_dict
				water_dict[(i,j,k)] = brick_water_dict


	return atom_entries, bond_entries, bond_entries_cshff, crystal_dict, water_dict
...
def get_angles(crystal_dict, water_dict, shape):
	...
	for cell in crystal_dict.keys():
		...
		for w in water_dict[cell]:
			...
			angle_entries_cshff.append( [angle_index_cshff, 1, water_dict[cell][w][1][7],  water_dict[cell][w][0][7],  water_dict[cell][w][2][7] ] ) #atom_index_cshffを読み込ませる	
			angle_index += 1
			angle_index_cshff += 1

	return angle_entries, angle_entries_cshff
...
def check_move_water_hydrogens(crystal_entries, water_model):
	...
	for itry in range(10):
		...
		for iwater in range(len(list_Ow)):
			...
			ok_struc = False
			new_molecule_coordinates(iwater, list_Hw, list_Ow, aux_entries, water_model)
			for i in range(100):
				#print(iwater, i)
...
def new_molecule_coordinates(iwater, list_Hw, list_Ow, aux_entries, water_model):
	...
	if water_model == "SPC_Fw": #SPC/Fwモデルは104.5（14.5）
	...
	elif water_model == "SPC": # SPCモデルは109.47（19.47）
		ang = np.random.rand()*2*np.pi
		x_p = np.cos(19.47*np.pi/180)*np.sin(ang)
		y_p = np.cos(19.47*np.pi/180)*np.cos(ang)
		z_p = -np.sin(19.47*np.pi/180)
	...
	aux_entries[iHw1][3:6] = np.array(aux_entries[list_Ow[iwater]][3:6]) + r_H1
	aux_entries[iHw2][3:6] = np.array(aux_entries[list_Ow[iwater]][3:6]) + r_H2
...
def check_new_molecule(iwater, list_Hw, list_Ow, list_oH, list_O, aux_entries, min_dist_H=0.8, min_dist_O=1.0):
	...
	for iH in range(len(list_oH)):
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		aux = np.concatenate(( aux, [np.array(aux_entries[list_oH[iH]][3:6]) - np.array(aux_entries[iHw][3:6])] ))
	...
	for iw in range(2*(iwater-1)):
		...
		aux = np.concatenate(( aux, [np.array(aux_entries[list_Hw[iw]][3:6]) - np.array(aux_entries[iHw][3:6])] ))
	...
	for iO in range(len(list_O)):
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		aux = np.concatenate(( aux, [np.array(aux_entries[list_O[iO]][3:6]) - np.array(aux_entries[iHw][3:6])] ))
	...
	for iO in range(len(list_Ow)):
		...
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		aux = np.concatenate(( aux, [np.array(aux_entries[list_Ow[iO]][3:6]) - np.array(aux_entries[iHw][3:6])] ))
...
def check_new_molecule_old(iwater, list_Hw, list_Ow, list_oH, list_O, aux_entries, min_dist_H=0.8, min_dist_O=1.0):
	...
	for iH in range(len(list_oH)):
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		dist = check_distance_PBC_old( np.array(aux_entries[list_oH[iH]][3:6]) , np.array(aux_entries[iHw][3:6]) )
		if dist < min_dist_H:
			return False
		iHw = 2*iwater + 1
		iHw = list_Hw[iHw]
	...
	for iw in range(2*(iwater-1)):
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		dist = check_distance_PBC_old( np.array(aux_entries[list_Hw[iw]][3:6]) , np.array(aux_entries[iHw][3:6]) )
		if dist < min_dist_H:
			return False
		iHw = 2*iwater + 1
		iHw = list_Hw[iHw]
	...
	for iO in range(len(list_O)):
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		dist = check_distance_PBC_old( np.array(aux_entries[list_O[iO]][3:6])  , np.array(aux_entries[iHw][3:6]) )
		if dist < min_dist_O:
			return False
	...
	for iO in range(len(list_Ow)):
		...
		iHw = 2*iwater
		iHw = list_Hw[iHw]
		dist = check_distance_PBC_old( np.array(aux_entries[list_Ow[iO]][3:6]) , np.array(aux_entries[iHw][3:6]) )
		if dist < min_dist_O:
			return False
